...
# ALL_METADATASET_NAMES = "aircraft".split(' ')
# ALL_METADATASET_NAMES = "mscoco".split(' ')
# ALL_METADATASET_NAMES = "aircraft cu_birds dtd quickdraw fungi vgg_flower traffic_sign mscoco mnist cifar10 cifar100 buaa".split(' ')


TRAIN_METADATASET_NAMES = "ilsvrc_2012 omniglot aircraft cu_birds dtd quickdraw fungi vgg_flower".split(' ')
TEST_METADATASET_NAMES = "traffic_sign mscoco mnist cifar10 cifar100 buaa".split(' ')
...
class MetaDatasetEpisodeReader(MetaDatasetReader):
    ...
    def __init__(self, mode, train_set=None, validation_set=None, test_set=None, test_type='standard'):
        ...
        if test_type == 'standard':
            ...
            if mode == 'test':
                test_episode_desscription = config.EpisodeDescriptionConfig(None, None, None, max_ways_upper_bound=15)
        ...
        # five-way-one-shot
            ...
            if mode == 'train':
                train_episode_desscription = config.EpisodeDescriptionConfig(None, None, None, min_ways=5, max_ways_upper_bound=5, max_num_query=10, max_support_size_contrib_per_class=5)
                self.train_dataset_next_task = self._init_multi_source_dataset(
                    train_set, SPLIT_NAME_TO_SPLIT['train'], train_episode_desscription)

            if mode == 'val':
                test_episode_desscription = config.EpisodeDescriptionConfig(None, None, None, min_ways=5, max_ways_upper_bound=5, max_num_query=10, max_support_size_contrib_per_class=4)
                for item in validation_set:
                    next_task = self._init_single_source_dataset(
                        item, SPLIT_NAME_TO_SPLIT['val'], test_episode_desscription)
                    self.validation_set_dict[item] = next_task
            ...
            if mode == 'test':
                test_episode_desscription = config.EpisodeDescriptionConfig(None, None, None, min_ways=5, max_ways_upper_bound=5, max_num_query=10, max_support_size_contrib_per_class=5)
                for item in test_set:
                    next_task = self._init_single_source_dataset(
                        item, SPLIT_NAME_TO_SPLIT['test'], test_episode_desscription)
                    self.test_set_dict[item] = next_task
