...
from models.DKD import dkd_loss
import geoopt
...
def train():
    ...
    optimizer_adaptor = geoopt.optim.RiemannianAdam([{'params': adaptors.parameters(), 'initial_lr': args['train.learning_rate']}],lr=0.1,weight_decay=5e-4)
    ...
    with tf.compat.v1.Session(config=config) as session:
        for i in tqdm(range(max_iter)):
            ...
            for t_indx, trainset in enumerate(trainsets):
                ...
                kd_p_losses_ = dkd_loss(logits[t_indx],stl_logits[t_indx],samples[t_indx]['labels'])
            ...
            batch_loss = torch.stack(batch_losses).sum() * 1.5
            kd_p_loss = kd_p_losses * args['train.beta']
            
            batch_loss = batch_loss + kd_p_loss
            batch_loss.backward()
            optimizer.step()
