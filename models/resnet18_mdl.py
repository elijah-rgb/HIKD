...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, classifier=None, num_classes=64,
        ...
        self.tp = hypnn.ToPoincare(c=args['test.c'],  ball_dim=512, riemannian=True)    
        ...
        if num_classes is not None:
            ...
            if classifier == 'linear':
                ...
                self.cls_fn = nn.ModuleList(cls_fn)
            elif classifier == 'bolic':
                for num_class in num_classes:
                    cls_fn.append(hypnn.HyperbolicMLR(ball_dim=512, n_classes=num_class, c=args['test.c']))
                self.cls_fn = nn.ModuleList(cls_fn)
                # print("mdlmlr ready!")
