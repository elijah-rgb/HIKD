...
class pa(nn.Module):
    ...
    def __init__(self, feat_dim):
        ...
        self.biolinear = HypLinear(feat_dim,feat_dim,args['test.c'])
    def forward(self, x):
...
class resnet_tsa(nn.Module):
    ...
    def reset(self):
        ...
        self.beta.biolinear.reset_parameters()    
...
def tsa(context_images, context_labels, model, max_iter=40, lr=0.1, lr_beta=1, distance='cos'):
    ...
    if 'beta' in tsa_opt:
        params.append({'params': beta_params, 'lr': 0.01})
    optimizer = geoopt.optim.RiemannianAdam(params,lr=0.0001) 
