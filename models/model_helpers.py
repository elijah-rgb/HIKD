...
from hyptorch.nn import ToPoincare,HypLinear
import geoopt
...
def get_model(num_classes, args):
    ...
    if 'pnf' in model_name:
        ...
        if args['model.pretrained']:
            ...
            base_network_path = os.path.join(args['source'], 'weights', base_network_name,
                                 'model_best.pth.tar')
            # base_network_path = os.path.join("/media/yangxilab/DiskB/kdc/tgrs/URL-master-DKD-bolic/saved_results/my_sdl/weights/imagenet-net/model_best.pth.tar")
            model_fn = partial(resnet18, dropout=dropout_rate,
                           pretrained_model_path=base_network_path)
        else:
            model_fn = partial(resnet18, dropout=dropout_rate)
...
def get_optimizer(model, args, params=None):
    ...
    optimizer = geoopt.optim.RiemannianAdam([{'params': params, 'initial_lr': args['train.learning_rate']}],lr=learning_rate,weight_decay=weight_decay)


    # optimizer = geoopt.optim.RiemannianAdam(params,lr=learning_rate,weight_decay=weight_decay)        
    return optimizer
