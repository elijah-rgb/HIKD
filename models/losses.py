...
def prototype_loss(support_embeddings, support_labels,
                   query_embeddings, query_labels, distance='bolic', c = args['test.c']):
    n_way = len(query_labels.unique())

    prots = compute_prototypes(support_embeddings, support_labels, n_way).unsqueeze(0)
    embeds = query_embeddings.unsqueeze(1)

    logits = -pmath.dist_matrix(torch.squeeze(embeds) , torch.squeeze(prots),c=args['test.c'])
...
def compute_prototypes(embeddings, labels, n_way):
    ...
    for i in range(n_way):
        if torch.__version__.startswith('1.1'):
            prots[i] = poincare_mean(embeddings[(labels == i).nonzero(), :], dim=0, c=args['test.c'])
        else:
            prots[i] = poincare_mean(embeddings[(labels == i).nonzero(as_tuple=False), :], dim=0, c=args['test.c'])
    return prots 
